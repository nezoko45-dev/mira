...
def ollama(history,memories,proactive=False,model='qwen3:4b'):
 clean=[]
 for x in history[-40:]:
 ...
 body=json.dumps({'model':model,'messages':[{'role':'system','content':system}]+clean,'stream':False,'options':{'temperature':0.8}}).encode(); req=Request('http://127.0.0.1:11434/api/chat',data=body,method='POST',headers={'content-type':'application/json'})
 try:
  with urlopen(req,timeout=180) as r: data=json.loads(r.read().decode())
 except HTTPError as e: raise RuntimeError(f'Ollama HTTP {e.code}: {e.read().decode(errors="replace")[:700]}')
 except Exception as e: raise RuntimeError('Ollama is not running. Install Ollama and pull qwen3:4b, then start Mira again. '+str(e))
 text=str(data.get('message',{}).get('content','')).strip(); mem=None
 if '[MEMORY:' in text:
...
def make_video(wav_bytes):
 ...
 for i in range(count):
  ...
  frame=a.copy(); shift=int(max(0,min(3,(level/threshold-0.8)*3))) if level>threshold else 0
...
class Handler(BaseHTTPRequestHandler):
 ...
 def do_GET(self):
  ...
  if p=='/health':
   try: urlopen('http://127.0.0.1:11434/api/tags',timeout=2); ollama_ok=True
   except Exception: ollama_ok=False
   return self.send_json(200,{'ok':True,'mira':True,'openvoice':True,'ollama':ollama_ok,'model':'qwen3:4b'})
 ...
 def do_POST(self):
  try:
   ...
   if path in ['/chat','/proactive']:
    reply,mem=ollama(data.get('history',[]),data.get('memories',[]),path=='/proactive'); result={'reply':reply,'memory':mem}
