...
async function deepgramSTT(audio,contentType,apiKey){
  ...
  const text=await r.text();if(!r.ok)throw new Error(`Deepgram STT ${r.status}: ${text.slice(0,1000)}`);
  const data=JSON.parse(text);return data?.results?.channels?.[0]?.alternatives?.[0]?.transcript||'';
}
async function deepgramTTS(text,apiKey){
  ...
  const b=Buffer.from(await r.arrayBuffer());if(!r.ok)throw new Error(`Deepgram TTS ${r.status}: ${b.toString('utf8').slice(0,1000)}`);return b;
}
...
async function animateWithFasterLivePortrait(audioBuffer){
  ...
  if(!r.ok)throw new Error(`FasterLivePortrait API ${r.status}: ${strFromU8(new Uint8Array(b)).slice(0,800)}`);
  return zipVideo(b);
}
async function flpOnline(){try{const r=await fetch(`${FLP_API}/docs`);return r.ok}catch{return false}}
async function makeReply(body){
  const dg=key(body,'deepgramKey','DEEPGRAM_API_KEY');if(!dg)throw new Error('Enter and save your Deepgram API key first.');
  const reply=lunaReply(body.text);const mp3=await deepgramTTS(reply,dg);const video=await animateWithFasterLivePortrait(mp3);
...
async function handle(req,res){
  try{
    ...
    if(req.method==='GET'&&url.pathname==='/health')return json(res,200,{ok:true,deepgram:true,fasterLivePortrait:await flpOnline(),javascriptMp4:false,museTalk:false,image:fs.existsSync(IMAGE),index:fs.existsSync(INDEX),fasterLivePortraitUrl:FLP_API});
...
server.on('error',e=>console.error('[Luna server]',e));
server.listen(PORT,'127.0.0.1',()=>{const url=`http://127.0.0.1:${PORT}`;console.log(`Luna is running at ${url}`);console.log(`FasterLivePortrait API expected at ${FLP_API}`);try{if(process.platform==='win32')exec(`start "" "${url}"`)}catch{}});
