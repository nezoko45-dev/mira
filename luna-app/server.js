...
async function getFLP(){if(!flpClientPromise)flpClientPromise=Client.connect(FLP_URL);try{return await flpClientPromise}catch(e){flpClientPromise=null;throw e}}
function endpointFromApi(info){const named=info?.named_endpoints||info?.namedEndpoints||{};for(const[name,spec]of Object.entries(named)){const p=JSON.stringify(spec?.parameters||spec?.inputs||[]).toLowerCase();if(p.includes('driving_audio')||p.includes('audio'))return name}for(const name of Object.keys(named))if(/animate|execute_video/i.test(name))return name;return'/gpu_wrapped_execute_video';}
function findVideo(value){if(!value)return null;if(typeof value==='string'&&/\.(mp4|webm|mov)(\?|$)/i.test(value))return value;if(typeof value==='object'){for(const k of['url','path','video','value']){const h=findVideo(value[k]);if(h)return h}for(const v of Object.values(value)){const h=findVideo(v);if(h)return h}}if(Array.isArray(value))for(const v of value){const h=findVideo(v);if(h)return h}return null}
async function animateWithFasterLivePortrait(audioBuffer,audioType){
  const client=await getFLP(); const info=await client.view_api(); const endpoint=endpointFromApi(info); const imageRef=handle_file(IMAGE); const audioRef=handle_file(new Blob([audioBuffer],{type:audioType||'audio/mpeg'}));
  const inputs=[imageRef,null,null,null,null,audioRef,'',false,true,true,1.0,true,false,false,false,'all',2.3,0.0,-0.125,2.2,0.0,-0.1,1e-7,'Image','Audio',4.0,'af_heart'];
  const result=await client.predict(endpoint,inputs); const video=findVideo(result?.data??result); if(!video)throw new Error(`FasterLivePortrait returned no video. Endpoint: ${endpoint}`); const vr=await fetch(video.startsWith('http')?video:new URL(video,FLP_URL).href); if(!vr.ok)throw new Error(`Could not read FasterLivePortrait output (${vr.status}).`); return Buffer.from(await vr.arrayBuffer());
}
async function makeReply(body){const dg=key(body,'deepgramKey','DEEPGRAM_API_KEY');if(!dg)throw new Error('Enter and save your Deepgram API key first.');const reply=lunaReply(body.text);const mp3=await deepgramTTS(reply,dg);const video=await animateWithFasterLivePortrait(mp3,'audio/mpeg');return{reply,audio_base64:mp3.toString('base64'),video_base64:video.toString('base64')};}

async function handle(req,res){try{const url=new URL(req.url,`http://127.0.0.1:${PORT}`);if(req.method==='OPTIONS')return send(res,204,'text/plain','');if(req.method==='GET'&&url.pathname==='/health'){let livePortrait=false;try{const r=await fetch(`${FLP_URL}/`);livePortrait=r.ok}catch{}return json(res,200,{ok:true,deepgram:true,fasterLivePortrait:livePortrait,javascriptMp4:false,museTalk:false,image:fs.existsSync(IMAGE),index:fs.existsSync(INDEX),fasterLivePortraitUrl:FLP_URL});}if(req.method==='GET'&&(url.pathname==='/'||url.pathname==='/index.html'))return fs.existsSync(INDEX)?send(res,200,'text/html; charset=utf-8',fs.readFileSync(INDEX)):json(res,404,{error:'index.html missing.'});if(req.method==='GET'&&url.pathname==='/luna.png')return fs.existsSync(IMAGE)?send(res,200,'image/png',fs.readFileSync(IMAGE)):json(res,404,{error:'luna mouth closed.png missing.'});if(req.method==='POST'&&url.pathname==='/stt'){const body=JSON.parse((await readBody(req)).toString('utf8'));const dg=key(body,'deepgramKey','DEEPGRAM_API_KEY');if(!dg)throw new Error('Enter and save your Deepgram API key first.');const audio=Buffer.from(String(body.audio_base64||''),'base64');if(!audio.length)throw new Error('No microphone audio received.');return json(res,200,{transcript:await deepgramSTT(audio,body.content_type,dg)});}if(req.method==='POST'&&url.pathname==='/reply'){const body=JSON.parse((await readBody(req)).toString('utf8'));return json(res,200,await makeReply(body));}if(req.method==='POST'&&url.pathname==='/voice-turn'){const body=JSON.parse((await readBody(req)).toString('utf8'));const dg=key(body,'deepgramKey','DEEPGRAM_API_KEY');if(!dg)throw new Error('Enter and save your Deepgram API key first.');const audio=Buffer.from(String(body.audio_base64||''),'base64');if(!audio.length)throw new Error('No microphone audio received.');const transcript=await deepgramSTT(audio,body.content_type,dg);if(!transcript.trim())return json(res,200,{transcript:'',reply:'',audio_base64:'',video_base64:''});const result=await makeReply({...body,text:transcript});return json(res,200,{transcript,...result});}return json(res,404,{error:'Not found'});}catch(e){console.error('[Luna]',e);return json(res,500,{error:e?.message||String(e)});}}

http.createServer(handle).listen(PORT,'127.0.0.1',()=>{console.log(`Luna is running at http://127.0.0.1:${PORT}`);console.log(`FasterLivePortrait expected at ${FLP_URL}`);try{if(process.platform==='win32')exec(`start "" "http://127.0.0.1:${PORT}"`)}catch{}});
