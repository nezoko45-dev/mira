...
const GENERATED = path.join(os.tmpdir(), 'luna-generated');
const MAX_BODY = 12 * 1024 * 1024;
const MUSETALK_SPACE = 'henrybit/musetalk-1-5';
const MUSETALK_BASE = 'https://henrybit-musetalk-1-5.hf.space';
...
function findVideoUrl(value, seen = new Set()) {
  ...
  if (typeof value === 'string') {
    ...
    if (/\.mp4($|\?)/i.test(value) || value.includes('/file=') || value.includes('/gradio_api/file=')) return value;
    return '';
  }
  if (typeof value !== 'object' || seen.has(value)) return '';
  seen.add(value);
  for (const k of ['url', 'video', 'path', 'file', 'value', 'href', 'data', 'output']) {
    if (value[k] != null) {
      const found = findVideoUrl(value[k], seen);
...
function normalizeVideoUrl(value) {
  const raw = findVideoUrl(value);
  if (!raw) throw new Error('MuseTalk finished but returned no MP4 URL. Raw result was: ' + JSON.stringify(value).slice(0, 1800));
  if (/^https?:\/\//i.test(raw)) return raw;
  if (raw.startsWith('/')) return MUSETALK_BASE + raw;
  if (/\.mp4($|\?)/i.test(raw)) return MUSETALK_BASE + '/' + raw.replace(/^\/+/, '');
  throw new Error('MuseTalk returned an unsupported video reference: ' + raw);
}

async function saveGeneratedVideo(remoteUrl) {
  const r = await fetch(remoteUrl);
  if (!r.ok) throw new Error('Could not download MuseTalk MP4 (' + r.status + ').');
  const contentType = r.headers.get('content-type') || '';
  const data = Buffer.from(await r.arrayBuffer());
  if (!data.length) throw new Error('MuseTalk returned an empty MP4.');
  if (contentType && !contentType.includes('video') && !remoteUrl.toLowerCase().includes('.mp4')) {
    console.warn('[MuseTalk] unexpected content type:', contentType);
  }
  const name = `luna-${Date.now()}-${Math.random().toString(16).slice(2)}.mp4`;
  fs.writeFileSync(path.join(GENERATED, name), data);
...
async function museTalk(audio) {
  ...
  try {
    ...
    console.log('[MuseTalk] result:', JSON.stringify(result).slice(0, 5000));
    const remoteUrl = normalizeVideoUrl(result?.data ?? result);
    console.log('[MuseTalk] video URL:', remoteUrl);
    return await saveGeneratedVideo(remoteUrl);
  } finally {
...
async function handle(req, res) {
  try {
    ...
    if (req.method === 'GET' && url.pathname === '/health') {
      return json(res, 200, { ok: true, deepgram: true, musetalk: true, image: fs.existsSync(IMAGE), model: MUSETALK_SPACE });
    }
    if (req.method === 'GET' && (url.pathname === '/' || url.pathname === '/index.html')) {
      const f = path.join(APP, 'index.html');
      return fs.existsSync(f) ? send(res, 200, 'text/html; charset=utf-8', fs.readFileSync(f)) : json(res, 404, { error: 'index.html missing' });
    }
    ...
    if (req.method === 'GET' && url.pathname.startsWith('/generated/')) {
      ...
      if (!/^luna-[a-z0-9-]+\.mp4$/i.test(name)) return json(res, 400, { error: 'Invalid video name.' });
      const f = path.join(GENERATED, name);
      return fs.existsSync(f) ? send(res, 200, 'video/mp4', fs.readFileSync(f)) : json(res, 404, { error: 'Generated video expired or was not found.' });
    }
    ...
    if (req.method === 'POST' && url.pathname === '/voice-turn') {
      ...
      const dg = key(body, 'deepgramKey', 'DEEPGRAM_API_KEY'); if (!dg) throw new Error('Add your Deepgram API key in the app.');
      const audio = Buffer.from(String(body.audio_base64 || ''), 'base64'); if (!audio.length) throw new Error('No microphone audio received.');
      const transcript = await deepgramSTT(audio, body.content_type || 'audio/webm', dg);
