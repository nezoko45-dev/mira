...
def make_wav(text):
    ...
    # Windows SAPI is available without downloading a model.
...
def make_video(wav_bytes):
    ...
    for i in range(count):
        ...
        activity = max(0.0, min(1.0, (level / baseline - 0.75) / 1.8))
        frame = img.copy()
        # Subtle audio-driven animation: breathing, vertical motion and glow.
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if p == '/health': return self.json(200, {'ok': True, 'app': 'Luna HTML App', 'video': IMAGE.exists()})
        if p in ('/', '/index.html'):
            f = ROOT / 'index.html'
